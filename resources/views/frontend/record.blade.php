...
document.querySelector('#uploadForm').addEventListener('submit',event=>{
    if(!input.files.length){event.preventDefault();error.textContent='Recording file is not ready. Please record again.';return}
    const sizeMb=input.files[0].size/1024/1024;
    if(sizeMb>48){event.preventDefault();error.textContent='Recording is too large to upload. Please record again.';return}
...
start.onclick=async()=>{try{error.textContent='';if(!stream)throw new Error('Camera is not ready');stopFaceDetection();zoomRange.disabled=true;zoomButtons.forEach(button=>button.disabled=true);for(let n=3;n>0;n--){count.textContent=n;await new Promise(r=>setTimeout(r,700))}count.textContent='';chunks=[];seconds=0;const mimeType=MediaRecorder.isTypeSupported('video/webm;codecs=vp8,opus')?'video/webm;codecs=vp8,opus':'video/webm';recorder=new MediaRecorder(stream,{mimeType,videoBitsPerSecond:500000,audioBitsPerSecond:64000});recorder.onerror=e=>{error.textContent='Recording failed: '+(e.error?.message||'please try again.');start.hidden=false;stop.hidden=true};recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};recorder.onstop=()=>{const blob=new Blob(chunks,{type:recorder.mimeType||'video/webm'});if(!blob.size){error.textContent='No video was recorded. Please try again.';return}video.srcObject=null;video.src=URL.createObjectURL(blob);video.controls=true;const dt=new DataTransfer();dt.items.add(new File([blob],'recording.webm',{type:blob.type}));input.files=dt.files;error.textContent='Recording ready ('+(blob.size/1024/1024).toFixed(1)+' MB).';retake.hidden=false;go.hidden=false};recorder.start(1000);start.hidden=true;stop.hidden=false;tick=setInterval(()=>{seconds++;timer.textContent='00:'+String(seconds).padStart(2,'0');if(seconds>=20)stop.click()},1000)}catch(e){error.textContent=e.message||'Recording could not start. Please reload and try again.';zoomRange.disabled=false;zoomButtons.forEach(button=>button.disabled=false)}};
